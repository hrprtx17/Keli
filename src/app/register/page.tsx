...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Auto sign-in so session is active for onboarding
      ...
      <div className="flex w-full md:w-[45%] lg:w-[40%] items-center justify-center p-6 sm:p-8 relative z-10 bg-white dark:bg-zinc-900 shadow-2xl shadow-black/[0.02] border-r border-gray-100 dark:border-zinc-800">
        <div className="w-full max-w-md space-y-8 py-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button 
              type="submit" 
              className="w-full h-11 min-h-[44px] rounded-xl bg-black dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-white hover:shadow-md font-semibold text-[14px] transition-all duration-200 active:scale-[0.99] mt-2 flex items-center justify-center gap-2" 
              disabled={loading}
            >
              {loading ? (
                <>
          ...
          <p className="text-center text-[13px] font-medium text-gray-500 dark:text-zinc-400">
            ...
            <Link href="/login" className="text-orange-600 hover:text-orange-700 hover:underline transition-colors min-h-[44px] inline-flex items-center">
              Sign in
            </Link>
          </p>

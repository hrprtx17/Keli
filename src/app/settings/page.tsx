...
import { 
  ...
  Building, Info, Link as LinkIcon, X, ShieldAlert
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleDelete = async () => {
    if (deleteConfirm !== workspace?.slug) {
       toast.error('Verification failed: Input text does not match workspace identity.');
       return;
    }
    setIsDeleting(true);
    try {
      ...
      toast.error('Failed to initiate clean wipe.');
      setIsDeleting(false);
    }
  };
            ...
            >
               <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                  <div>
                     ...
                     <p className="text-[13px] text-red-700/70 mt-1 leading-relaxed">
                        Permanently eliminate this workspace environment and all its associated intelligent systems.
                     </p>
                  </div>
                  <button 
                    onClick={() => setShowDeleteModal(true)}
                    className="px-6 py-2.5 rounded-xl border border-red-200 text-red-600 bg-white hover:bg-red-50 text-[13px] font-semibold transition-all shrink-0 hover:shadow-sm active:scale-95"
                  >
                     Delete Workspace
                  </button>
               </div>
            ...
            {showDeleteModal && (
               ...
               >
                  ...
                  >
                     ...
                     <h3 className="text-[18px] font-semibold text-gray-900 tracking-tight">Confirm Complete Deletion</h3>
                     <p className="text-[13px] text-gray-500 mt-2 leading-relaxed">
                        This operation cascades immediately. All agents, conversation histories, and neural data associated with this environment will be irrevocably destroyed.
                     </p>

                     <div className="mt-6 bg-gray-50 border border-gray-100 rounded-xl p-4">
                        <p className="text-[12px] font-medium text-gray-600 mb-3">Type <span className="font-mono bg-gray-200 text-gray-800 px-1.5 py-0.5 rounded text-[11px]">{workspace?.slug}</span> to verify:</p>
                        <input 
                          type="text" 
                          value={deleteConfirm}
                          ...
                          placeholder="Enter workspace identity code"
                        />
                     </div>
                     ...
                     <div className="grid grid-cols-2 gap-3 mt-8">
                        ...
                        <button 
                          disabled={deleteConfirm !== workspace?.slug || isDeleting}

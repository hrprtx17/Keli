(function() {
  try {
    ...
    // Stream response from API (Streaming Resilient SSE Parser - FIX 3)
    ...
    // Ticket form (Defensive and safe forms - FIX 4)
    function showTicketForm() {
      ...
      if (subBtn) {
        subBtn.onclick = function() {
          // Re-query every element inside click handler to ensure latest references (FIX 4)
          ...
          .catch(function() {
            // Re-query elements inside promise catch to keep it bullet-proof
            var subEl = el('agd-tsub');
            if (subEl) {
